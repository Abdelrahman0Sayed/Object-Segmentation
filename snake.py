...
class ActiveContour(ImageProcessor):
    def __init__(self):
        super().__init__()
        self.center = (400, 350)
        self.radius = 100
        self.numOfIterations = 100
        self.alpha = 9  # Internal energy weight (elasticity/continuity)
        self.beta = 9   # External energy weight (image forces)
        self.gamma = 1  # Balloon energy weight (inflation/deflation)
        # Initialize gradient images as None
        self.gradient_x = None
        self.gradient_y = None

    def initial_contour(self, center=None, radius=None):
        """Generate initial circular contour points"""
        if center is not None:
            self.center = center
        if radius is not None:
    ...
    def calcInternalEnergy(self, pt, prev_pt, next_pt, alpha):
        """Calculate internal energy based on curvature"""
        dx1 = pt[0] - prev_pt[0]
        dy1 = pt[1] - prev_pt[1]
        dx2 = next_pt[0] - pt[0]
    ...
    def calcBalloonEnergy(self, pt, prev_pt, gamma):
        """Calculate balloon energy to inflate/deflate contour"""
        dx = pt[0] - prev_pt[0]
        dy = pt[1] - prev_pt[1]
        return gamma * (dx*dx + dy*dy)
    ...
    def display_output_contour(self, snake_contour, output_image=None):
        """Render the final contour on the image"""
        if self.img_original is None or snake_contour is None:
            return None
